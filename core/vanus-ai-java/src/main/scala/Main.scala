...
object Main:
  private val greeting = "Hello, how are you doing"
  ...
  def main(args: Array[String]): Unit =
    ...
    args.toList match
      case Nil | "info" :: Nil =>
        ...
        println("Commands: demo [steps] [checkpoint] | train <tiny|20m> <pairs.tsv> <steps> <checkpoint> | chat <checkpoint> <prompt>")
      case "demo" :: rest if rest.length <= 2 =>
        train(ModelConfig.tiny(), Vector(greeting -> "I am fine"), rest.headOption.fold(200)(_.toInt),
      ...
      case "train" :: size :: input :: steps :: output :: Nil =>
        ...
        val pairs = Files.readAllLines(Path.of(input)).asScala.filter(_.nonEmpty).zipWithIndex.map { (line, index) =>
          val fields = line.split("\t", -1)
          require(fields.length == 2 && fields.forall(_.nonEmpty), s"Line ${index + 1}: expected prompt<TAB>answer")
          fields(0) -> fields(1)
        }.toVector
        train(config, pairs, steps.toInt, Path.of(output))
  ...
  private def requireJava21(): Unit =
    ...
    if feature != 21 then
      throw new IllegalStateException(
        s"Vanus requires a Java 21 runtime; current runtime is Java $feature"
      )
  ...
  private def train(config: ModelConfig, pairs: Vector[(String, String)], steps: Int, output: Path): Unit =
