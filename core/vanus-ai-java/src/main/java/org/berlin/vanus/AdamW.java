...
public final class AdamW {
    ...
    // Per-parameter first (mean) and second (uncentered variance) moment buffers,
    ...
    public AdamW(Collection<Tensor> parameters, float decay) {
        if (!Float.isFinite(decay) || decay < 0)
            throw new IllegalArgumentException("Invalid decay");
        this.parameters = List.copyOf(parameters);
        this.decay = decay;
        for (Tensor p : parameters) {
            if (p.grad == null)
                throw new IllegalArgumentException("Optimizer needs trainable tensors");
            first.add(new float[p.data.length]);
            second.add(new float[p.data.length]);
        }
    }
    ...
    public double step(float learningRate, float maxNorm) {
        if (!Float.isFinite(learningRate) || learningRate <= 0 || !Float.isFinite(maxNorm) || maxNorm <= 0)
            throw new IllegalArgumentException("Invalid optimizer options");
        ...
        if (!Double.isFinite(norm))
            throw new IllegalStateException("Nonfinite gradients; update aborted");
        // Scale factor that rescales gradients down to the max norm, or 1 if already
        // within bounds.
        ...
        for (int p = 0; p < parameters.size(); p++) {
            Tensor t = parameters.get(p);
            float[] m = first.get(p), v = second.get(p);
            for (int i = 0; i < t.data.length; i++) {
                float g = t.grad[i] * clip;
                // Exponential moving averages of the gradient (first moment) and its square
                // (second moment).
                m[i] = 0.9f * m[i] + 0.1f * g;
    ...
    void write(DataOutput out) throws IOException {
        ...
        for (int p = 0; p < parameters.size(); p++) {
            ...
            for (float value : first.get(p))
                out.writeFloat(value);
            for (float value : second.get(p))
                out.writeFloat(value);
        }
    }

    static AdamW read(DataInput in, Collection<Tensor> parameters) throws IOException {
        float decay = in.readFloat();
        int steps = in.readInt(), count = in.readInt();
        ...
        for (int p = 0; p < count; p++) {
            ...
            if (length != optimizer.first.get(p).length)
                throw new IOException("Optimizer shape mismatch");
            for (int i = 0; i < length; i++)
                optimizer.first.get(p)[i] = finite(in.readFloat());
            for (int i = 0; i < length; i++)
                optimizer.second.get(p)[i] = finite(in.readFloat());
        }
        return optimizer;
    }
    ...
    private static float finite(float value) throws IOException {
        if (!Float.isFinite(value))
            throw new IOException("Nonfinite optimizer state");
        return value;
    }
}
